...
export class AuthService {
  async register(data: RegisterDto) {
    ...
    const accessToken = generateAccessToken({ id: String(created.id), email: created.email });
  ...
  async login(data: LoginDto) {
    ...
    const accessToken = generateAccessToken({ id: String(user.id), email: user.email });
    const refreshToken = generateRefreshToken();
